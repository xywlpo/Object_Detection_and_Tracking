...
class CSV(Data.Dataset):
    ...
    def __init__(self, config, stage):
        ...
        self.images_dir = os.path.join(self.dataset_dir, "train/")
        self.ann_path = os.path.join(self.dataset_dir, "train.csv")
        ...
        self.df = pd.read_csv(self.ann_path)
        self.images_ids = self.df['image_id'].unique()
    ...
    def __getitem__(self, item):
        ...
        :return: 样本及其标注信息
        """
        # 获取CSV文件中某个图像id号对应的全部信息
        image_id = self.images_ids[item]
        records = self.df[self.df['image_id'] == image_id]
        num_objs = min(len(records), self.max_objs)

        # 按照原始图像最长边将图像缩放到统一尺寸(resolution)，采用仿射变换实现
        img = cv2.imread(self.images_dir+str(image_id)+'.jpg')
        ...
        boxes = np.stack(records['bbox'].apply(lambda x: np.fromstring(x[1:-1], sep=',')))
        for k in range(num_objs):
            # 获取bbox和class_id
            box = boxes[k]
            bbox = self.ltwhBOX_to_ltrbBOX(box)
            class_id = 0
